...
try {
  ...
  if (fs.existsSync(VERIFICATIONS_FILE)) {
    ...
    verificationStore = new Map(Object.entries(data));
    console.log(`✅ Loaded ${verificationStore.size} pending verifications`);
  }
} catch (e) {
...
app.get("/api/check-username", (req, res) => {
  ...
  const existingUser = userStore.get(uid);

  // A username is ONLY "taken" if a VERIFIED user owns it
  const isTaken = existingUser && existingUser.isVerified === true;
...
app.post("/api/signup", async (req, res) => {
  ...
  // 1. Check if UID taken by a VERIFIED user
  const existingByUid = userStore.get(uid);
  if (existingByUid && existingByUid.isVerified === true) {
    return res.status(400).json({ success: false, error: "Username already taken" });
  }

  // 2. Check if Email taken by a VERIFIED user
  for (const u of userStore.values()) {
    const uEmail = (u.email || "").toLowerCase().trim();
    if (uEmail === normalizedEmail && u.isVerified === true) {
      return res.status(400).json({ success: false, error: "Email already registered" });
    }
  }

  // NOTE: If an unverified user exists with this UID or Email, we ALLOW the signup
  // to proceed. They will overwrite the unverified record once they verify the new OTP.
...
io.on("connection", (socket) => {
  ...
  socket.on("send-message", async ({ to, message, from }) => {
    ...
    if (to === "ai_friend") {
      ...
      let userText = decryptMessage(message.text);
    ...
    const fullMsg = {
      ...
      size: message.size,         // ✅ Required for large file skip-decrypt check
      encrypted: message.encrypted, // ✅ Required for receiver to know it's encrypted
      iv: message.iv,              // ✅ Required for AES-GCM decryption
      version: message.version,    // ✅ Required to pick correct decryption method
